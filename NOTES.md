...
- 2026-07-23: Repo revealed as the full frontier-lab journey (3 pillars: infra=root, system-design/, behaviour/ — each own teach workspace). README rewritten around that. .gitignore added (user then hand-tuned it — respect their version).
- 2026-07-23: GEORGIAN track added — lessons/ka/*.html, all 21 pages translated (prose/quizzes Georgian, code+terms English, lang="ka"), EN↔KA toggle pill on every page (.lang-switch in course.css). Verified: links, quiz data-correct parity, Georgian script present. Reference cards remain English-only for now — translate on request.
