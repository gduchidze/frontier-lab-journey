...
## Working notes
- 2026-07-23 (later): **Phase 2 authored in full (lessons 0010–0017)** — EN + KA both, per explicit user request (supersedes the earlier same-day Georgian pause, at least for this phase). Files: `lessons/0010-monolith-to-microservices` · `0011-api-design-deep` · `0012-storage-engine-internals` · `0013-streams-and-events` · `0014-distributed-transactions` · `0015-reliability-patterns` · `0016-observability-and-ops` · `0017-security-and-multitenancy` (each `.html` + `.ka.html`). Labs: `lab-0010-microservice-tax` (latency/availability/tail-at-scale of decomposition), `lab-0011-idempotency-api` (double-charge + pagination drift), `lab-0012-lsm-vs-btree` (write/read amplification), `lab-0013-mini-kafka` (partitions, consumer groups, delivery semantics, compaction), `lab-0014-sagas-and-fencing` (dual-write, saga compensation, fencing tokens), `lab-0015-circuit-breaker` (retry storms, breaker, load shedding), `lab-0016-observability-lab` (RED, SLO/error budget, burn-rate alerts, trace), `lab-0017-tenant-isolation` (JWT verify, noisy neighbor, tenant-leak drill). index.html Phase 2 section added; 0009→0010 nav fixed (was stale "0010 Rate Limiter" from v1 roadmap).
